...
export const Mapping: FunctionComponent = () => {
  ...
  useEffect(() => {
    ...
    const handle = (data: {
      ...
      const routeData = routes.find((route) => route._id === data.routeId);
      if (data.finished && routeData) finishRoute(routeData);
    };

    socketIORef.current.on("new-position", handle);

    return()=>{
      socketIORef.current?.off('new-position', handle)
    }
